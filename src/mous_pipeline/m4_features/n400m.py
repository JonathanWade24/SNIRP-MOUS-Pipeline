...
def n400m_amplitude(
    ...
    *,
    ...
    np.savez(
        ...
        sensor_prefix=np.asarray(sensor_prefix),
        tmin=np.asarray(float(tmin)),
        tmax=np.asarray(float(tmax)),
        has_topography_weights=np.asarray(topography_weights is not None),
